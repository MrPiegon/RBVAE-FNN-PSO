...
import methods
from data.molecule_iterator import SmileSingleIterator
...
class Vae(nn.Module):
    def __init__(self, vocab, vocab_size, embedding_size, dropout, padding_idx,
        ...
        num_dec_layers = (2 if bidirectional else 1) * n_layers
    ...
    def encoder_sample(self, input_sequence, epsilon_std=1.0):
        ...
        # sample prior
        z = methods.sample(mean, logv, (batch_size, self.latent_size), epsilon_std).cuda()
        return input_embedding, mean, logv, z

    def simple_encode(self, input_smiles):
        """ Encode for non-batch smiles."""
        smi_iterator = SmileSingleIterator(input_smiles, self.vocab, vocab_direct_read=True)
        iterator = smi_iterator.return_iter()
        for batch in iterator:
            x = batch.smile[0]
            # input sequence are like: <sos> SMILES tokens <padding>
            # for input, remove the last token
            input_sequence = x[:, :-1]
            _, _, _, z = self.encoder_sample(input_sequence)
    ...
    def calc_mi(self, x):
        ...
        log_qz = methods.log_sum_exp(log_density, dim=1) - math.log(x_batch)
...
class Decoder(nn.Module):
    ...
    def inference_direct(self, latent, embedding, max_len=None):
        if max_len is None:
            max_len = self.max_len
        """ Make sure that latent is tensor."""
        assert latent.size(1) == self.latent_size, 'latent size error!'
        batch_size = latent.size(0)
        hidden = self.latent2hidden(latent)
        ...
        # index_pred_view = index_pred.cpu().detach().numpy()
        smiles_pred = []
        for i in range(batch_size):
        ...
        assert len(smiles_pred) == index_pred.size(0)
        return smiles_pred, index_pred
